...
Vagrant.configure(2) do |config|
  config.vm.box = "trusty64-hwy61-10032016"
  config.vm.box_url = "https://oss-binaries.phusionpassenger.com/vagrant/boxes/latest/ubuntu-14.04-amd64-vbox.box"
  config.hostmanager.enabled = true
  config.hostmanager.manage_host = true
  config.hostmanager.ignore_private_ip = false
  config.hostmanager.include_offline = true
  config.vm.hostname = 'trusty64-hwy61-10032016'
  ...
  config.vm.network "forwarded_port", guest: 3000, host: 3000 # noda dev server
  ...
  # 'dev' directory below holds the github code for our application `hwy61`
  # on the guess box it will be located @ /usr/local/src/dev
  ...
  config.vm.provision "ansibleLocal" do |ansible|
    ...
    ansible.limit = "all"
    ansible.playbook = CWD+"/ops/provision_hwy61.yml"
  end
